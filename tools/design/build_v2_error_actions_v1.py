#!/usr/bin/env python3
"""Normalize generated error sheets into the locked 48x64 V2 runtime contract."""
...
def validate_approved_lock() -> dict:
    ...
    if "syncing" not in actions:
        raise RuntimeError("Approved V3 baseline does not include syncing")
    mismatches = []
    ...
    if mismatches:
        raise RuntimeError(f"Approved V3 baseline changed: {mismatches[:10]}")
    return {
...
def normalize_frame(source: Path, visible_height: int) -> Image.Image:
    ...
    width = min(46, max(1, round(subject.width * visible_height / subject.height)))
...
def build_character(role: str, spec: dict) -> tuple[list[Image.Image], dict]:
    action_dir = SPRITE_ROOT / spec["directory"] / "actions/v1/error"
    ...
    for index in range(1, 5):
        source = processor_dir / f"error-{index}.png"
        frame = normalize_frame(source, spec["visible_height"])
        output = frames_dir / f"{spec['prefix']}-error-wheelbase-v2-{index}-48x64.png"
        frame.save(output)
        frames.append(frame)
    ...
    strip_path = action_dir / f"{spec['prefix']}-error-4frame-wheelbase-v2-strip-48x64.png"
    sheet_path = action_dir / f"{spec['prefix']}-error-4frame-wheelbase-v2-2x2-48x64.png"
    gif_path = previews_dir / f"{spec['prefix']}-error-wheelbase-v2-48x64.gif"
    preview_gif = previews_dir / f"{spec['prefix']}-error-wheelbase-v2-preview-6x.gif"
    preview_png = previews_dir / f"{spec['prefix']}-error-wheelbase-v2-preview-6x.png"
    ...
    if any(item["visible_magenta_pixels"] or item["touches_edge"] for item in qcs):
        raise RuntimeError(f"Error frame QC failed for {role}: {qcs}")
    if len({sha256(path) for path in output_paths}) != 4:
        raise RuntimeError(f"Error frames are not distinct for {role}")

    source_prompt = action_dir / "source" / f"{spec['prefix']}-error-wheelbase-v2-magenta-raw.prompt.txt"
    metadata_path = action_dir / f"{spec['prefix']}-error-4frame-wheelbase-v2-meta.json"
    metadata = {
        "asset": f"{role}_child_error",
        "revision": "v2-wheelbase-error-v1",
        ...
        "source_raw": relative(action_dir / "source" / f"{spec['prefix']}-error-wheelbase-v2-magenta-raw.png"),
...
def build_trio(character_frames: dict[str, list[Image.Image]]) -> None:
    trio_dir = SPRITE_ROOT / "trio/actions/v1/error"
    previews = trio_dir / "previews"
    previews.mkdir(parents=True, exist_ok=True)
    phases = []
    for phase in range(4):
        canvas = Image.new("RGBA", (FRAME_SIZE[0] * 3, FRAME_SIZE[1]), (0, 0, 0, 0))
        for column, role in enumerate(CHARACTERS):
            canvas.alpha_composite(character_frames[role][phase], (column * FRAME_SIZE[0], 0))
        phases.append(canvas)

    preview_png = previews / "kindergarten-ai-agent-trio-error-v2-preview-6x.png"
    preview_gif = previews / "kindergarten-ai-agent-trio-error-v2-preview-6x.gif"
    phases[0].resize((864, 384), Image.Resampling.NEAREST).save(preview_png)
    save_gif(phases, preview_gif, scale=6)

    metadata = {
        "asset": "kindergarten_ai_agent_trio_error",
        "revision": "v2-wheelbase-error-v1",
        "status": "visual_approval_candidate",
        "action": "error",
    ...
    (trio_dir / "kindergarten-ai-agent-trio-error-v2-meta.json").write_text(
        json.dumps(metadata, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
...
def build_state_action_preview(character_frames: dict[str, list[Image.Image]]) -> None:
            ...
            if action == "error":
                ...
                row.append(
                    Image.open(
                        ...
                        / action
                        / "frames"
                        / f"{spec['prefix']}-{action}-wheelbase-v2-1-48x64.png"
                    ).convert("RGBA")
                )
        rows.append(row)

    preview = Image.new("RGBA", (48 * 3, 64 * len(actions)), (0, 0, 0, 0))
    for row_index, row in enumerate(rows):
        for column, frame in enumerate(row):
            preview.alpha_composite(frame, (column * 48, row_index * 64))
    preview_path = action_root / "kindergarten-ai-agent-trio-state-actions-v4-preview-6x.png"
    preview.resize((864, 64 * len(actions) * 6), Image.Resampling.NEAREST).save(preview_path)

    metadata = {
        "asset": "kindergarten_ai_agent_trio_state_actions",
        "revision": "v2-wheelbase-state-actions-v4",
        "status": "visual_approval_candidate",
    ...
    (action_root / "kindergarten-ai-agent-trio-state-actions-v4-meta.json").write_text(
        json.dumps(metadata, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
