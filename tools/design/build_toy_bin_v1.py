#!/usr/bin/env python3
"""Build the first production toy storage bin and complete-prop QA preview."""
...
PROP_DIR = ROOT / "assets/design/maps/classroom-corner/props/toy-bin/v1"
...
RAW = PROP_DIR / "toy-bin-green-raw.png"
CLEAN = PROP_DIR / "toy-bin-green-clean.png"
PROP = PROP_DIR / "toy-bin-40x48.png"
PROP_PREVIEW = PROP_DIR / "toy-bin-preview-8x.png"
...
BLOCK_TABLE = PROPS_DIR / "block-table/v1/block-table-96x56.png"
QA_PREVIEW = ART_DIR / "classroom-corner-all-props-qa-preview-512x288.png"
META = PROP_DIR / "toy-bin-meta.json"
...
def build_prop() -> tuple[Image.Image, dict]:
    ...
    ys, xs = np.where(keep)
    if not len(xs):
        raise RuntimeError("Clean toy bin has no visible component")
...
def compose_qa(prop: Image.Image, layout: dict) -> None:
    ...
    preview.alpha_composite(Image.open(BLOCK_TABLE).convert("RGBA"), (352, 56))
...
def main() -> None:
    ...
    meta = {
        ...
        "superseded_by": "toy-bin-v2-left-to-right",
        ...
        "prompt": str((PROP_DIR / "toy-bin-green-raw.prompt.txt").relative_to(ROOT)),
