#!/usr/bin/env python3
"""Approve the V2 syncing state and extend the immutable animation lock to v3."""
...
PREVIOUS_LOCK_PATH = LOCK_DIR / "v2-wheelbase-animation-baseline-lock-v2.json"
LOCK_PATH = LOCK_DIR / "v2-wheelbase-animation-baseline-lock-v3.json"
SYNCING_EXTENSION = (
    MAP_ROOT
    / "extensions/syncing/v1/classroom-corner-syncing-extension-v1.json"
)
SYNCING_PROP_META = (
    MAP_ROOT / "props/sync-mail-station/v1/sync-mail-station-meta.json"
)
TRIO_SYNCING_META = (
    SPRITE_ROOT
    / "trio/actions/v1/syncing/kindergarten-ai-agent-trio-syncing-v2-meta.json"
)
STATE_ACTIONS_META = (
    SPRITE_ROOT
    / "trio/actions/v1/kindergarten-ai-agent-trio-state-actions-v3-meta.json"
)
STATE_ACTIONS_PREVIEW = (
    SPRITE_ROOT
    / "trio/actions/v1/kindergarten-ai-agent-trio-state-actions-v3-preview-6x.png"
)
...
def syncing_metadata_paths() -> list[Path]:
    return [
...
def validate_character_metadata(role: str, path: Path) -> dict[str, Any]:
    ...
    if metadata.get("status") not in {"visual_approval_candidate", "approved"}:
        raise RuntimeError(f"Unexpected syncing status: {relative(path)}")
    if metadata.get("action") != "syncing" or metadata.get("frames") != 4:
        raise RuntimeError(f"Unexpected syncing contract: {relative(path)}")
    if metadata.get("runtime_frame_px") != FRAME_SIZE:
        raise RuntimeError(f"Unexpected frame size: {relative(path)}")
    if (
        metadata.get("visible_wheel_bottom_y_exclusive")
        != VISIBLE_WHEEL_BOTTOM_Y_EXCLUSIVE
    ):
    ...
    if (
        not qc.get("technical_qc_passed")
        or not qc.get("all_frames_distinct")
        or len(frame_qc) != 4
    ):
        raise RuntimeError(f"Syncing technical QC failed: {relative(path)}")
    for item in frame_qc:
        if (
            item.get("size_px") != FRAME_SIZE
            or item.get("visible_height_px") != expected_height
            or item.get("wheel_bottom_y_exclusive")
            != VISIBLE_WHEEL_BOTTOM_Y_EXCLUSIVE
            or item.get("visible_magenta_pixels") != 0
            or item.get("touches_edge")
        ):
            raise RuntimeError(f"Syncing frame QC failed: {relative(path)}")

    runtime_frames = metadata.get("runtime_frames", [])
    if len(runtime_frames) != 4 or any(
        not (ROOT / frame).exists() for frame in runtime_frames
    ):
        raise RuntimeError(f"Syncing runtime frames are missing: {relative(path)}")
    if len({sha256(ROOT / frame) for frame in runtime_frames}) != 4:
        raise RuntimeError(f"Syncing runtime frames are not distinct: {relative(path)}")
...
def validate_syncing_assets() -> dict[str, Any]:
    characters = {
        role: validate_character_metadata(role, character_metadata_path(role))
        for role in CHARACTERS
    }

    trio = read_json(TRIO_SYNCING_META)
    if trio.get("status") not in {"visual_approval_candidate", "approved"}:
        raise RuntimeError("Unexpected trio syncing approval status")
    if (
        trio.get("action") != "syncing"
        or trio.get("frames") != 4
        or not trio.get("technical_qc_passed")
    ):
        raise RuntimeError("Trio syncing technical QC failed")
    ...
    if "syncing" not in aggregate.get("actions", []) or not aggregate.get(
        "technical_qc_passed"
    ):
        raise RuntimeError("State-action bundle does not contain approved syncing QC")
    if not STATE_ACTIONS_PREVIEW.exists():
        raise RuntimeError("State-action v3 preview is missing")

    return {
        "characters": characters,
        "trio_metadata": relative(TRIO_SYNCING_META),
        "state_actions_metadata": relative(STATE_ACTIONS_META),
        "technical_qc_passed": True,
    }


def validate_syncing_extension() -> dict[str, Any]:
    extension = read_json(SYNCING_EXTENSION)
    if extension.get("status") not in {"visual_approval_candidate", "approved"}:
        raise RuntimeError("Syncing map extension is not ready for approval")
    validation = extension.get("validation", {})
    if (
        not validation.get("passed")
        or not validation.get("syncing_actions", {}).get("passed")
        or not validation.get("routes_and_collisions", {}).get("passed")
    ):
        raise RuntimeError("Syncing map extension validation failed")

    prop = read_json(SYNCING_PROP_META)
    if prop.get("status") not in {
        "visual_approval_candidate",
        "approved_visual",
    }:
        raise RuntimeError("Sync mail station is not ready for approval")
    if not prop.get("qc", {}).get("technical_qc_passed"):
        raise RuntimeError("Sync mail station technical QC failed")
    if not prop.get("collision") or not prop.get("trigger_zone"):
        raise RuntimeError("Sync mail station collision or trigger zone is missing")

    return {
        "manifest": relative(SYNCING_EXTENSION),
        "prop_metadata": relative(SYNCING_PROP_META),
...
def approve_metadata() -> list[str]:
    approved = []
    for path in syncing_metadata_paths():
        metadata = read_json(path)
    ...
    write_json(SYNCING_PROP_META, prop)
    ...
    write_json(SYNCING_EXTENSION, extension)
    return approved
...
def approved_asset_paths(previous_lock: dict[str, Any]) -> list[Path]:
    ...
    roots = [
        *(
        ...
        SPRITE_ROOT / "trio/actions/v1/syncing",
    ]
    paths.update(
        path
        for root in roots
...
def main() -> None:
    ...
    syncing_validation = validate_syncing_assets()
    extension_validation = validate_syncing_extension()
    ...
    manifest = {
        "lock_id": "v2-wheelbase-animation-baseline-lock-v3",
        "status": "approved",
        ...
        "approval_evidence": {
            ...
            "syncing_assets": syncing_validation,
            "syncing_extension": extension_validation,
        ...
        "extension_policy": (
            "New states such as error must use new candidate metadata and a new "
            "lock revision; files or hashes in this approved lock are immutable."
        ),
    }
