#!/usr/bin/env python3
"""Build the first production reading book-bin prop and map QA preview."""
...
PROP_DIR = ROOT / "assets/design/maps/classroom-corner/props/reading-book-bin/v1"
BOOKSHELF_DIR = (
    ROOT / "assets/design/maps/classroom-corner/props/reading-bookshelf/v1"
)
...
RAW = PROP_DIR / "reading-book-bin-green-raw.png"
CLEAN = PROP_DIR / "reading-book-bin-green-clean.png"
PROP = PROP_DIR / "reading-book-bin-48x40.png"
PROP_PREVIEW = PROP_DIR / "reading-book-bin-preview-8x.png"
BOOKSHELF = BOOKSHELF_DIR / "reading-bookshelf-96x44.png"
QA_PREVIEW = ART_DIR / "classroom-corner-reading-props-qa-preview-512x288.png"
META = PROP_DIR / "reading-book-bin-meta.json"
...
def build_prop() -> tuple[Image.Image, dict]:
    ...
    ys, xs = np.where(keep)
    if not len(xs):
        raise RuntimeError("Clean reading book bin has no visible component")

    source_box = (int(xs.min()), int(ys.min()), int(xs.max()) + 1, int(ys.max()) + 1)
    subject = resize_rgba_premultiplied(clean.crop(source_box), RUNTIME_SIZE)
...
def main() -> None:
    ...
    meta = {
        "prop_id": "reading-book-bin-v1",
        "status": "approved_visual",
        "classification": ["compact_prop", "collision_bearing_object"],
        "asset_strategy": "one_by_one",
        "view": "mostly front-facing 3/4 interior prop",
        "perspective": {
            "camera_position": "slightly_right_of_object",
            "look_direction": "right_to_left",
            "visible_side": "right",
            "far_side": "left",
        },
        "source_reference": str(
            (PROP_DIR / "reading-book-bin-raw.png").relative_to(ROOT)
        ),
        "raw": str(RAW.relative_to(ROOT)),
        "prompt": str(
            (PROP_DIR / "reading-book-bin-green-raw.prompt.txt").relative_to(ROOT)
        ),
        ...
        "collision": {
            "type": "rect",
            "x": MAP_TOP_LEFT[0],
            "y": MAP_TOP_LEFT[1] + RUNTIME_SIZE[1] - 6,
            "w": RUNTIME_SIZE[0],
            "h": 6,
        },
        "depth_sort": {
            ...
            "actor_behind_when": "actor_sort_y < 84",
            "actor_in_front_when": "actor_sort_y >= 84",
        },
        "processing": {
